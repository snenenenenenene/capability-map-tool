...
export default class EditStatus extends Component {
    ...
    async componentDidMount() {
        await axios.get(`http://localhost:8080/api/status/${this.state.statusId}`)
        .then(response => this.setState({validityPeriod: response.data.validityPeriod}))
            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol className="breadcrumb">
                        ...
                        <li className="breadcrumb-item"><Link to={`/environment/${this.state.environmentName}/status`}>Status</Link></li>
